...
def create_country_code_mappings():
    """
    Cria dicionário para buscar o código numérico do país a partir do nome
    com diferentes variações (nome exato, minúsculas, sem 'the', etc)
    """
    # Dicionário para mapear nomes de países para códigos
    name_to_code = {}
    
    # Adiciona todas as variações dos nomes oficiais
    for code, data in countries.items():
        ...
        if country_name:
            ...
            # Nome em minúsculas
            name_to_code[country_name.lower()] = code
            
            # Nome sem "the" no início, se aplicável
            if country_name.lower().startswith("the "):
                name_to_code[country_name.lower()[4:]] = code
    
    # Adiciona aliases
    for alias, main_name in country_aliases.items():
        # Tenta encontrar o código pelo nome principal
        if main_name in name_to_code:
            name_to_code[alias] = name_to_code[main_name]
            name_to_code[alias.lower()] = name_to_code[main_name]
            
            # Se o alias começa com "the", adiciona versão sem "the"
            if alias.lower().startswith("the "):
                name_to_code[alias.lower()[4:]] = name_to_code[main_name]
    
    return name_to_code

def get_country_code(country_name, name_to_code):
    """
    Tenta encontrar o código do país usando diferentes variações do nome
    """
    if not country_name or country_name == "null":
        return None
    
    # Tenta com o nome exato como está
    if country_name in name_to_code:
        return name_to_code[country_name]
    
    # Tenta com o nome em minúsculas
    if country_name.lower() in name_to_code:
        return name_to_code[country_name.lower()]
    
    # Tenta sem "the" no início
    if country_name.lower().startswith("the "):
        name_without_the = country_name.lower()[4:]
        if name_without_the in name_to_code:
            return name_to_code[name_without_the]
...
def translate_countries(input_file, output_file):
    """
    Traduz os nomes de países no arquivo JSONL para seus códigos numéricos
    """
    name_to_code = create_country_code_mappings()
    output_lines = []
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            print(f"Total de linhas no arquivo de entrada: {len(lines)}")
            
            for i, line in enumerate(lines, start=1):
                line = line.strip()
                if not line:
                    print(f"Linha {i} vazia, ignorada.")
                    continue
                
                try:
                    player_data = json.loads(line)
                    
                    if i == 1 and isinstance(player_data, list) and "playerName" in player_data:
                        output_lines.append(json.dumps(player_data, ensure_ascii=False))
                        print("Cabeçalho identificado e preservado.")
                        continue
                    ...
                    if isinstance(player_data, list):
                        ...
                        # Processa nationality1
                        if len(player_data) > nationality1_idx:
                            original_name = player_data[nationality1_idx]
                            if isinstance(original_name, (int, str)) and str(original_name).isdigit():
                                print(f"Linha {i}: Mantido código numérico '{original_name}' em nationality1")
                            elif original_name and original_name != "null":
                                code = get_country_code(original_name, name_to_code)
                                if code:
                                    print(f"Linha {i}: Traduzido '{original_name}' -> '{code}'")
                                    player_data[nationality1_idx] = code
                                else:
                                    country_not_found.add(original_name)
                                    print(f"Linha {i}: País '{original_name}' não encontrado no dicionário!")
                        
                        # Processa nationality2
                        if len(player_data) > nationality2_idx:
                            original_name = player_data[nationality2_idx]
                            if isinstance(original_name, (int, str)) and str(original_name).isdigit():
                                print(f"Linha {i}: Mantido código numérico '{original_name}' em nationality2")
                            elif original_name and original_name != "null":
                                code = get_country_code(original_name, name_to_code)
                                if code:
                                    print(f"Linha {i}: Traduzido '{original_name}' -> '{code}'")
                                    player_data[nationality2_idx] = code
                                else:
                                    country_not_found.add(original_name)
                                    print(f"Linha {i}: País '{original_name}' não encontrado no dicionário!")
                        ...
                        print(f"AVISO: Linha {i} não é uma lista como esperado: {type(player_data)}")
                        output_lines.append(line)
                
                except json.JSONDecodeError as e:
                    print(f"ERRO: Linha {i} não é um JSON válido: {e}")
                    output_lines.append(line)
                except Exception as e:
                    print(f"ERRO inesperado na linha {i}: {str(e)}")
                    output_lines.append(line)
        ...
        if country_not_found:
            print(f"\nPaíses não encontrados no dicionário ({len(country_not_found)}):")
            for country in sorted(country_not_found):
                print(f"  - {country}")
        else:
            print("\nTodos os países foram traduzidos com sucesso!")
...
if __name__ == "__main__":
    input_file = "players.jsonl"  # Arquivo de entrada
    output_file = "new_players.jsonl"  # Arquivo de saída
    
    # Se houver argumentos na linha de comando, usa-os
    if len(sys.argv) > 1:
